	...
	if ($_SESSION['rank'] >= rank_speak)
	{
		if (isset($_POST['action']))
		{
			...
			if ($_POST['action'] == 'send' && isset($_POST['msg']) && strlen($_POST['msg']) > 0)
			{
				...
				$insert = $db->prepare("INSERT INTO chatbox_mj VALUES ('', NOW(), ?, ?, ?, ?)");
			...
			else if ($_POST['action'] == 'delete' && isset($_POST['msg']))
			{
				...
				$answer= $db->prepare('SELECT user_id FROM chatbox_mj WHERE id = ?');
				...
				if ($line = $answer->fetch())
				{
					...
					{
						$delete = $db->prepare('DELETE FROM chatbox_mj WHERE id = ?');
			...
			else if ($_POST['action'] == 'reload')
			{
				...
				$answer = $db->prepare('SELECT COUNT(*) AS number FROM chatbox_mj WHERE ((to_id = 0 AND salon = ?) OR to_id = ? OR (user_id = ? AND to_id != 0))');
