(function () {
	...
	function deleteMsg (msg) {
		if (confirm('Voulez-vous vraiment supprimer ce message ? Il ne sera pas supprimé pour les utilisateurs l\'ayant déjà vu.')) {
			xhrDel.open('POST', 'index.php?p=chatboxsystem');
	...
	function reload () {
		...
		xhr.open('POST', 'index.php?p=chatboxsystem');
	...
	function send () {
		if (sendForm.msg.value != '') {
			...
			xhrSend.open('POST', 'index.php?p=chatboxsystem');
			xhrSend.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

			var to = (sendForm.to.value) ? '&to=' + sendForm.to.value : '';
			var salon = (sendForm.salon.value) ? '&salon=' + sendForm.salon.value : '';

			xhrSend.send('action=send&msg=' + sendForm.msg.value + to + salon);
